...
async def get_contact(db: AsyncSession, contact_id: int, user_id: int):
	result = await db.execute(select(Contact).filter(Contact.user_id == user_id, Contact.id == contact_id))
	return result.scalar()


async def get_contacts(db: AsyncSession, user_id: int, skip: int = 0, limit: int = 10):
	result = await db.execute(select(Contact).filter(Contact.user_id == user_id).offset(skip).limit(limit))
...
async def update_contact_in_db(db: AsyncSession, user_id: int, contact_id: int, contact: ContactUpdate):
	try:
		result = await db.execute(select(Contact).filter(Contact.user_id == user_id, Contact.id == contact_id))
...
async def search_contacts_in_db(db: AsyncSession, user_id: int, query: str):
	result = await db.execute(
		select(Contact).filter(Contact.user_id == user_id).filter(
			or_(
...
async def get_birthdays_within_next_week(db: AsyncSession, user_id: int):
	...
	result = await db.execute(
		select(Contact).filter(Contact.user_id == user_id, Contact.birthday.between(today, next_week))
	)
	return result.scalars().all()
